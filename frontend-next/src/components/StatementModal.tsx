...
import { X, Calendar, FileText, ChevronDown, AlertCircle, Wallet, Banknote, Layers, Loader2 } from 'lucide-react';
...
export function StatementModal({
  ...
  useEffect(() => {
    ...
    if (periodType === 'custom' && (!startDate || !endDate || dateError)) {
    ...
    transactionsAPI
      ...
      .then((res) => {
        if (!cancelled) setStatementData(res.data);
      })
      .catch((err) => {
        console.error('Erro ao buscar extrato:', err);
        if (!cancelled) {
          setStatementData(null);
          setStatementError('Não foi possível carregar o extrato.');
        }
      })
      .finally(() => {
        if (!cancelled) setIsLoadingStatement(false);
      });
  ...
  }, [isOpen, selectedGroup, paymentFilter, periodType, startDate, endDate, dateError]);
  ...
  const canGeneratePDF = () => {
    ...
    if (isLoadingStatement || statementError || !statementData) return false;
  ...
  const getPDFDocument = () => {
    ...
    return (
      <StatementPDF
        ...
        saldoInicial={statementData.saldoInicial}
        totalEntradas={statementData.totalEntradas}
        totalSaidas={statementData.totalSaidas}
        saldoFinal={statementData.saldoFinal}
  ...
  const entriesCount = statementData?.movements.filter((m) => m.type === 'ENTRADA').length ?? 0;
  const expensesCount = statementData?.movements.filter((m) => m.type === 'SAIDA').length ?? 0;
  const periodBalance = statementData ? statementData.totalEntradas - statementData.totalSaidas : 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
              ...
              <>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-red-600 font-medium">
                    {expensesCount} registros
                  </span>
                </div>
                <div className="flex justify-between text-sm mt-2 pt-2 border-t border-gray-200">
                  <span>Saldo do período:</span>
                  <span className={`font-medium ${periodBalance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(periodBalance)}
                  </span>
                </div>
              </>
            )}
        ...
        <div className="flex gap-3 p-4 border-t border-gray-200">
          ...
          {isPDFReady ? (
            <PDFDownloadLink
              key={`${selectedGroup}-${periodType}-${startDate}-${endDate}-${paymentFilter}`}
            ...
            >
              {isLoadingStatement ? 'Carregando...' : 'Baixar Extrato'}
            </button>
          )}
        </div>
      </div>
